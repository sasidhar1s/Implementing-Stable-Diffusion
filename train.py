...
def train_model(rank, world_size, device_id, config):
    ...
    if is_main_process:
        exp_dir = f"experiments/twofiftysix_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    dataset = DiffusionDataset(image_dir=config['paths']['image_dir'], 
                            captions_file=config['paths']['captions_file'], 
                            image_size=config['model_parameters']['image_size'],
                            caption_dropout_prob=config['training_parameters']['caption_dropout_prob'])
    
    sampler = DistributedSampler(dataset, num_replicas=world_size, rank=rank, shuffle=True)
    
    
    dataloader = DataLoader(dataset, batch_size=config['training_parameters']['batch_size_per_gpu'], 
    ...
    Denoising_model = DenoisingModel(in_channels=config['model_parameters']['latent_channels'],
                                    context_dim=config['model_parameters']['context_dim']).to(device)
    
    text_encoder = CLIPTextEncoder(model_name=config['model_parameters']['image&text_encoding_model'],
                                device=device)
    ...
    optimizer = optim.AdamW(Denoising_model.parameters(), 
                            ...
                            weight_decay=config['training_parameters'].get('weight_decay', 1e-2),
                            betas=(0.9, 0.999))

    total_iterations = config['training_parameters']['total_iterations']
    
    warmup_iterations = config['training_parameters'].get('warmup_iterations',
                                                          0.1 * total_iterations)
    ...
    scaler = GradScaler(enabled=config['training_parameters'].get('mixed_precision', True))
    
    ema = ExponentialMovingAverage_EMA(Denoising_model,
                                    decay=config['training_parameters'].get('ema_decay', 0.9999))

    Denoising_model = DDP(Denoising_model, device_ids=[device_id])
    
    iteration, epoch = load_checkpoint(config['paths'].get("resume_checkpoint"),
    ...
    while iteration < total_iterations:
        ...
        for batch,batch_idx in enumerate(dataloader):
            ...
            indices = torch.randperm(timesteps, device=device)[:batch_size]
            t = indices[torch.randint(0, len(indices), (batch_size,), device=device)]
            ...
            with autocast(device_type='cuda', enabled=config['training_parameters'].get('mixed_precision', True)):
            ...
            if (batch_idx + 1) % accumulation_steps == 0:
                ...
                torch.nn.utils.clip_grad_norm_(Denoising_model.parameters(),
                                            config['training_parameters'].get('max_grad_norm', 1.0))
                scaler.step(optimizer)
                scaler.update()
                optimizer.zero_grad(set_to_none=True)
                scheduler.step()
                ema.update(Denoising_model.module)
                iteration += 1

                        
            if world_size > 1:
                dist.all_reduce(mse_loss, op=dist.ReduceOp.AVG)

            if is_main_process and iteration % config['sampling_parameters']['print_frequency'] == 0:
                logging.info(f"Epoch [{epoch}], Iteration [{iteration}/{total_iterations}], Loss: {mse_loss.item():.4f}, LR: {scheduler.get_last_lr()[0]:.6f}")
                        
                
            if is_main_process and iteration > 0 and iteration % config['sampling_parameters']['save_frequency'] == 0:
                ema.apply_shadow(Denoising_model.module if isinstance(Denoising_model, DDP) else Denoising_model)
                save_path = os.path.join(exp_dir, 'checkpoints', f"model_iteration_{iteration}.pth")
                model_state = Denoising_model.module.state_dict() if isinstance(Denoising_model, DDP) else Denoising_model.state_dict()
                ...
                ema.restore(Denoising_model.module)
                
            if is_main_process and iteration % config['sampling_parameters']['sample_frequency'] == 0:
                ...
                ema.restore(Denoising_model.module if isinstance(Denoising_model, DDP) else Denoising_model)
...
def generate_validation_samples(diffusion_model, denoising_model,
                                text_encoder, vae_model, config, 
                                device, iteration, exp_dir):
    denoising_model.eval()
    
    with open(config['paths']['validation_prompts_file'], 'r') as f:
        validation_prompts = json.load(f)
    
    sample_dir = os.path.join(exp_dir, 'samples') 
    
    with torch.no_grad():
        cfg_scale = config['sampling_parameters'].get('cfg_scale', 1.0) 
        ...
        sampled_latents = sample_with_cfg(diffusion_model, denoising_model,
                                        cond_embeddings, uncond_embeddings,cfg_scale, config, device,
                                        config['sampling_parameters'].get('clip_denoised',True), 
                                        config['sampling_parameters'].get('repeat_noise', False))
...
def sample_with_cfg(diffusion_model, denoising_model, cond_context, uncond_context, 
    ...
    for i in reversed(range(config['diffusion_schedule']['timesteps'])):
        t = torch.full((num_samples,), i, device=device, dtype=torch.long)
        
        
        with torch.no_grad(), autocast(device_type="cuda",enabled=config['training_parameters'].get('mixed_precision', True)):
